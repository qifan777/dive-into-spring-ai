...
type AiFactory = AiFactoryDto['AiFactoryRepository/FETCHER']
type AiFactoryScope = Scope<AiFactory>
const table = ref<TableInstance>()
const pageData = ref<Page<AiFactory>>()
const initQuery: AiFactorySpec = {}
const queryRequest = ref<QueryRequest<AiFactorySpec>>({
  pageNum: 1,
  pageSize: 10,
  query: { ...initQuery }
})
const loadData = async ({
  ...
  pageData.value = await api.aiFactoryController.query({
...
const handleBatchDelete = () => {
  handleDelete(
    table.value?.getSelectionRows().map((row: AiFactory) => {
      return row.id || ''
    })
  )
}
const handleDelete = (ids: string[]) => {
  ElMessageBox.confirm('此操作将删除数据且无法恢复, 是否继续?', '警告', {
    ...
    api.aiFactoryController.delete({ body: ids }).then((res) => {})
  })
}
const dialogVisible = ref(false)
const initForm: AiFactoryInput = { name: '', value: '' }
const form = ref<AiFactoryInput>({ ...initForm })
const formRef = ref<FormInstance>()

const rules = reactive<FormRules<AiFactoryInput>>({
  name: [{ required: true, message: '请输入名称', trigger: 'blur' }],
  value: [{ required: true, message: '请输入英文名称', trigger: 'blur' }]
})

const handleEdit = (row: AiFactory) => {
...
const handleSubmit = () => {
  formRef.value?.validate(
    assertFormValidate(() =>
      api.aiFactoryController.save({ body: form.value }).then(async (res) => {
        form.value.id = res
  ...
  <div>
    ...
    <el-table ref="table" :data="pageData?.content" :border="true" height="500">
      ...
      <el-table-column label="名称" prop="name" show-overflow-tooltip width="120">
        <template v-slot:default="{ row }: AiFactoryScope">
          {{ row.name }}
        </template>
      </el-table-column>
